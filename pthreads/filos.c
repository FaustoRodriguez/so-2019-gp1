...
void *life(void *ptId) {
	...
	while (1){
		...
		actions(); printf("Tengo hambre %ld\n", tid);
		if (tid % 2 == 0) {
			pthread_mutex_lock(&forks[tid]);
			printf("Ya tengo el tendor dercho %ld\n", tid);
			pthread_mutex_lock(&forks[(tid + 1) % NUM_FILOS]);
			printf("Ya tengo el tenedor izquierdo %ld\n", tid);
			printf("Ha comer %ld\n", tid);
			actions(); printf("Regresando tenedores %ld\n", tid);
			pthread_mutex_unlock(&forks[tid]); pthread_mutex_unlock(&forks[(tid + 1) % NUM_FILOS]);
		} else {
			pthread_mutex_lock(&forks[(tid + 1) % NUM_FILOS]);
			printf("Ya tengo el tenedor izquierdo %ld\n", tid);
			pthread_mutex_lock(&forks[tid]);
